...
prepared_data = SETTINGS.DATA + '/COORDINATES_Pred_Price.csv'
PRICE_MODEL_PATH0 = SETTINGS.MODEL + '/price#0.h5'
PRICE_MODEL_PATH1 = SETTINGS.MODEL + '/price#1.h5'
PRICE_MODEL_PATH2 = SETTINGS.MODEL + '/price#2.h5'
...
def Model_0(data: pd.DataFrame):
    ...
    X_train = ds0.drop(['price'], axis=1).values
    #ds0["price"] = np.log1p(ds0["price"])
    y_train = ds0["price"] / maxPrice
    ...
    model_filepath = PRICE_MODEL_PATH0
...
def Model_1(data: pd.DataFrame):
    ...
    X_train = ds1.drop(['price'], axis=1).values
    # ds0["price"] = np.log1p(ds0["price"])
    y_train = ds1["price"] / maxPrice
    ...
    model.compile(optimizer=opt, loss='mean_squared_logarithmic_error', metrics=['mse'])
    # Fitting the ANN to the training set
    model_filepath = PRICE_MODEL_PATH1
    checkpoint = ModelCheckpoint(model_filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
def Model_2(data: pd.DataFrame):
    ...
    X_train = ds2.drop(['price'], axis=1).values
    # ds0["price"] = np.log1p(ds0["price"])
    y_train = ds2["price"] / maxPrice
    ...
    model.compile(optimizer=opt, loss='mean_squared_logarithmic_error', metrics=['mse'])
    # Fitting the ANN to the training set
    model_filepath = PRICE_MODEL_PATH2
